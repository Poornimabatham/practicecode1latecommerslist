...
<<<<<<< HEAD
 ...
 const lateComersList = await Database.from("AttendanceMaster as A").select(
  Database.raw("SUBSTRING_INDEX(A.EntryImage, '.com/', -1) as EntryImage"),
  'E.FirstName',
  'E.LastName',
  'A.TimeIn as atimein',
  Database.raw("TIMEDIFF(A.TimeIn,CASE WHEN(S.TimeInGrace!='00:00:00') THEN S.TimeInGrace ELSE S.TimeIn END) as latehours"),
  'A.ShiftId'
)
